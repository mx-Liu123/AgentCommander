...
*   **Transparent Debugging (White-box)**: A major pain point with many agent frameworks is their "black-box" nature. In AgentCommander, every experiment runs in its own folder. You can simply `cd` into any experiment directory and run `gemini -r`, `qwen -c`, or `opencode -c` to **resume the conversation**. This allows you to inspect the **final formatted prompt** received by the CLI, review the history, and manually intervene if the agent gets stuck.
